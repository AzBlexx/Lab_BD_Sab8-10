...
DELETE from SALIDAS where nombre= 'Gilberto Alejandro'

go

CREATE FUNCTION cantSal(@id int)
RETURNS TABLE
AS
RETURN (
SELECT cantidad FROM SALIDAS WHERE id_producto=@id
)
go

SELECT * FROM dbo.cantSal(1)
go

CREATE TRIGGER borrar2
on dbo.SALIDAS
after DELETE
AS
BEGIN
...
DELETE from SALIDAS where id_producto=3
...
DELETE FROM SALIDAS WHERE id_producto=2

SELECT * FROM SALIDAS
GO

CREATE PROCEDURE todosLosQueTengan10
@num int
as
SET NOCOUNT ON;
SELECT * FROM PRODUCTOS
...
EXECUTE todosLosQueTengan10 10
go


CREATE PROCEDURE ClientesFac
@folio int
as
SET NOCOUNT ON;
SELECT CLIENTES.nombre, CLIENTES.apellidoM, CLIENTES.apellidoP, CLIENTES.id_cliente AS 'Cliente', FACTURAS.fecha_factura FROM CLIENTES left join FACTURAS ON CLIENTES.id_cliente=FACTURAS.id_cliente
WHERE CLIENTES.id_cliente=@folio
go

EXECUTE ClientesFac 1
go


CREATE PROCEDURE ProveedorDeProductoPorCant 
@cant int
as
SET NOCOUNT ON;
SELECT p.nombre AS 'Nombre del Producto' , p.cant_disp, p.precio, pr.nombre as 'Nombre del proveedor'   FROM PRODUCTOS p RIGHT JOIN PROVEEDOR as pr ON p.id_provedor=pr.id_provedor
WHERE P.cant_disp=@cant
go

EXECUTE ProveedorDeProductoPorCant 10
GO

CREATE PROCEDURE GananciaArribaDeAlgo
@cant int
AS
set nocount on;
SELECT SUM(v.monto) as 'Ganancia', P.folio, P.fecha_hora FROM PAGO p INNER JOIN VENTAS v on p.id_pago=v.id_pago 
GROUP BY p.folio, p.fecha_hora
HAVING SUM(v.monto) > @cant
go


EXECUTE GananciaArribaDeAlgo 99
GO
